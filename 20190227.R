...
all_HR_weme <- all_HR_weme[!(all_HR_weme$difference) > 1, ]
...
wemeType_plot <- ggplot(wemeType_HR, 
  ...
  ylab("Heartrate (bpm)")+
  #theme(axis.text.x = element_text(angle=-90, hjust=1))+
  scale_x_discrete(labels=as.factor(index(unique(wemeType_HR$userId))))
print(wemeType_plot)
...
HR_medians <- ggplot(HRvsUser, aes(x=index, y=median_HR, colour=userId, group = userId)) +
  geom_point() + geom_line() +
  theme_bw() +
...
total_median <- ggplot(HRvsUser_summary, aes(x=userId, y=median_HR)) +
...
#Delta HR @ exercise
wemeDHR_plot <- ggplot(all_HR_weme, 
                        aes(x=animationName, y=delta_HR_daily)) +
  geom_point() +
  theme_bw() +
  xlab("Exercise")+
  ylab("Delta HR (bpm)")+
