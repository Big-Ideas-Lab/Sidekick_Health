...
HR_noOut_summary <- group_by(no_HR_outliers, userId) %>% 
  dplyr::summarize(average_HR = mean(value), 
            lower_quartile_HR= quantile(value, 0.25), stan_dev=sd(value))
summary(HR_noOut_summary)

noOut_avgHR<- no_HR_outliers %>% mutate(date = ymd_hms(date), 
           day = floor_date(date, 'day')) %>%
  group_by(userId, day) %>%
  dplyr::summarize(avg_HR = mean(value), lower_quartile_HR= quantile(value, 0.25))

noOut_avgHR <- data.table(noOut_avgHR)
noOut_avgHR[,index:= order(day), by="userId"]

##title: Resting HR per User
HR_noOut_plot <- ggplot(HR_noOut_summary, 
...
##title: HR per day per user No Outliers (lol needs work)
plotHR_noOut <- ggplot(noOut_avgHR, 
                       aes(x=index, y=avg_HR, colour=userId, group = userId)) +
  geom_point() + geom_line() +
  theme_bw() +
  xlab("Number of days") +
  ylab("Heart rate (beats/min)")
...
# SDvsRestHR <- ggplot(HR_noOut_summary, 
#                         aes(x=lower_quartile_HR, y=stan_dev))+
#   geom_point()+ 
#   geom_smooth(method='lm', se=FALSE)+
#   theme_bw()+
#   xlab("Resting HR (bpm)")+
#   ylab("Standard Deviation of HR")
# print(SDvsRestHR)
...
all_User$`0`<- NULL
UserInfo_HR <- merge(HR_noOut_summary, all_User, by.x= "userId", by.y = "id")

#title: Trend between BMI and Average Resting HR 
BMIvsRestHR <- ggplot(UserInfo_HR,
                        aes(x=BMI, y=lower_quartile_HR))+
  geom_point()+
  geom_smooth(method='lm', se=FALSE)+
  theme_bw()+
  xlab("BMI")+
