...
#all_Mission <- read.table(file="/Users/emiliagrzesiak/Downloads/results-20181018-230557.csv",
                          #header=TRUE, sep=",")
all_Mission <- read.table(file="/Users/emiliagrzesiak/Downloads/20190221",
                          header=TRUE, sep=",")
...
HRvsUser_summary <- group_by(all_HR, userId) %>% 
  dplyr::summarize(average_HR = mean(value), median_HR=median(value), 
...
#resting HR per week per user --> for delta HR later
# need to change if more than 1 year of data
quartile1HR <- quartile1HR %>% 
  mutate(week = week(day))

restHR_weekly <- quartile1HR %>% 
  group_by(userId, week) %>%
  dplyr::summarize(resting_HR = mean(value), stan_dev=sd(value))
restHR_weekly <- data.table(restHR_weekly)
restHR_weekly[,index_week:= order(week), by="userId"]

##title: Resting HR per week
weeklyHR_plot <- ggplot(restHR_weekly, aes(x=index_week, y=resting_HR, colour=userId))+
  geom_point()+ geom_line()+
  theme_bw()+
  xlab("Week #")+
  ylab("Resting HR (bpm)")+
...
# boxplot_weeklyHR <- ggplot(quartile1HR,
#                       aes(x = format(day, format("%U")),
#                           y = value, group = format(day, format("%U")))) +
#   geom_boxplot(fill = "grey80", colour = "#3366FF") +
#   stat_summary(fun.data = n_fun, geom = "text",
#                fun.y = median, colour = "black", size= 2, fontface = "bold") +
#   theme_bw()+
#   #stat_summary(fun.data = mean.n, geom = "text", fun.y = mean, colour = "red") +
#   facet_grid(userId~.)
# print(boxplot_weeklyHR)

#delta HR stuff
all_HR <- all_HR %>% 
  mutate(week = week(day))

delta_HR <- merge(all_HR[,c("date", "userId", "value", "week", "day")], 
                  restHR_weekly, by=c("userId", "week"))
delta_HR <- delta_HR %>%
  mutate(delta_HR = value - resting_HR)

boxplot_deltaHR <- ggplot(delta_HR,
                      aes(x = userId,
                          y = delta_HR, group =  userId, colour=userId)) +
  geom_boxplot() +
  stat_summary(fun.data = n_fun, geom = "text",
               fun.y = median, colour = "black", size= 2, fontface = "bold") +
  theme_bw()+
  theme(axis.text.x = element_text(angle=-90, hjust=1))+
  scale_color_hue(labels= index(unique(UserInfo_HR$userId)))+
  scale_x_discrete(labels=index(unique(UserInfo_HR$userId)))
  #stat_summary(fun.data = mean.n, geom = "text", fun.y = mean, colour = "red") +
print(boxplot_deltaHR)

#delta HR correlations
summary_deltaHR <- delta_HR %>%
  group_by(userId) %>%
  dplyr::summarize(avg_DeltaHR=mean(delta_HR), stan_dev=sd(delta_HR))

BMI_deltaHR <- merge(summary_deltaHR, 
                      UserInfo_HR[, c("userId", "BMI")], 
                     by = "userId")
cor.test(BMI_deltaHR$avg_DeltaHR, BMI_deltaHR$BMI)

fit4 <- lm(avg_DeltaHR ~ BMI, data = BMI_deltaHR)
deltaHRvsBMI <- ggplot(BMI_deltaHR, 
                        aes(x=BMI, y=avg_DeltaHR, colour=userId))+
  geom_point()+
  theme_bw()+
  stat_smooth(method = 'lm', aes(group = 'linear'), se = FALSE) +
  xlab("BMI")+
  ylab("Average Delta HR")+
  #scale_color_hue(labels= index(unique(UserInfo_HR$userId)))+
  labs(title = paste("R^2 = ",signif(summary(fit4)$adj.r.squared, 3), ",",
                     " P =",signif(summary(fit4)$coef[2,4], 4)))
print(deltaHRvsBMI)

#REAL TIME EXERCISE ANALYSIS!!!
weme_exercise_cat= subset(all_Mission, categoryId==102, 
                         select = c(id,animationName,typeId))
weme_exercise_cat$animationName=as.character(weme_exercise_cat$animationName)
colnames(weme_exercise_cat)[1] <- "missionId"
weme_exercise_cat <- subset(weme_exercise_cat, typeId==201)
weme_ID <- merge(weme_exercise_cat, 
                     all_ScoreDetail[, c("missionId","scoreId")], 
                     by="missionId", all.x = FALSE)
weme_time <- merge(weme_ID, all_Score[, c("scoreId", "date", "kicks", "userId")], 
                       by="scoreId", all.x = FALSE)

##Frequency of exercise input in app per user
weme_time <- weme_time[weme_time$userId %in% all_HR$userId,]
weme_time$day <- as.Date(weme_time$date, format="%Y-%m-%d")
weme_time <- weme_time[weme_time$day %in% all_HR$day,]
freq_weme <- weme_time %>% 
  group_by(userId, day) %>%
  dplyr::summarize(entries_on_day=n())
freq_weme <- data.table(freq_weme)
freq_weme[,index:= order(day), by="userId"]
freq_weme$userId <- as.character(freq_weme$userId)

total_weme <- weme_time %>% 
  group_by(userId, animationName) %>%
  dplyr::summarize(entries_total=n())
total_weme <- data.table(total_weme)
total_weme$userId <- as.character(total_weme$userId)

#Title: Weme over time
freq_weme_plot<- ggplot(freq_weme,
                            aes(x=index, y=entries_on_day, colour=userId, group=userId))+
  geom_point() + geom_line() +
  theme_bw() +
  ggtitle("Number of exercise entires per day per user") +
  ...
  #theme(axis.text.x = element_text(angle=-90, hjust=1))
  scale_color_hue(labels= index(unique(UserInfo_HR$userId)))
print(freq_weme_plot)

#Title: weme exercise frequency
wemeFreq_plot <- ggplot(total_weme, 
                            aes(x=userId, y=entries_total, fill=animationName)) +
  geom_col(position = position_dodge2(width = 1.0, preserve = "single")) +
  theme_bw() +
  xlab("User")+
  ylab("# of times done")+
  theme(axis.text.x = element_text(angle=-90, hjust=1))
...
#combining weme exercise + HR data
all_HR <- merge(all_HR, noOut_avgHR, by=c("userId", "day"))
all_HR <- all_HR %>%
  mutate(delta_HR_daily = value - lower_quartile_HR)
weme_time$date <- as.POSIXct(weme_time$date, format="%Y-%m-%d %H:%M:%S")
z_weme <- lapply(intersect(all_HR$userId, weme_time$userId),function(ID) {
  HR1 <- subset(all_HR[, c("value", "date", "userId", "delta_HR_daily")],userId==ID)
  EX1 <- subset(weme_time[, c("animationName", "userId", "date", "scoreId")],userId==ID)
  EX1$indices <- sapply(EX1$date,function(d) which.min(abs(HR1$date - d)))
  HR1$indices <- 1:nrow(HR1)
  merge(HR1,EX1,by=c('userId','indices'))
})
all_HR_weme <- do.call(rbind,z_weme)
all_HR_weme$indices <- NULL
all_HR_weme$difference<- difftime(all_HR_weme$date.x, 
                                      all_HR_weme$date.y, units=c("days"))
all_HR_weme <- data.table(all_HR_weme)
all_HR_weme[,index:= order(date.x), by="userId"]
all_HR_weme <- all_HR_weme[(all_HR_weme$index) > 1, ]
all_HR_weme <- all_HR_weme[(!(all_HR_weme$difference) < -.003), ]
all_HR_weme <- all_HR_weme[(!(all_HR_weme$difference) > .005), ]

wemeType_HR <- all_HR_weme %>% 
  group_by(userId, animationName) %>%
  dplyr::summarize(average_HRperActivity= mean(value), avg_DHR= mean(delta_HR_daily))

wemeType_freq <- all_HR_weme %>% count(animationName, userId)
wemeType_HR <- merge(wemeType_HR, wemeType_freq, by=c("userId", "name"))
colnames(wemeType_HR)[4]<-"times_performed_byUser"

#title: Heartrate after weme exercise for each user
wemeType_plot <- ggplot(wemeType_HR, 
                            aes(x=userId, y=average_HRperActivity, fill=animationName)) +
  geom_col(position = position_dodge2(width = 1.0, preserve = "single")) +
  theme_bw() +
  xlab("User")+
  ylab("Heartrate (bpm)")+
...
#looking closely at HR during 1 exercise
weme_jack_sub1 <- subset(all_HR, userId == 6755366276694823)
weme_jack_sub1 <- weme_jack_sub1 %>% filter(date > as.POSIXct("2018-02-07 21:00:00") & date < as.POSIXct("2018-02-08 00:00:00"))

weme_jacks <- weme_jack_sub1 %>%
  mutate(highlight_flag = ifelse(date == as.POSIXct("2018-02-07 22:14:05"), T, F)) %>% 
  ggplot(aes(x=date, y=value)) +
  geom_point(aes(color = highlight_flag), show.legend = FALSE) +
  scale_color_manual(values = c('#595959', 'red'))+
  theme_bw() +
  ggtitle("HR before, during, after sideplanks") +
  xlab(" ") +
  ylab(" ")
print(weme_jacks)

weme_jack_sub2 <- subset(all_HR, userId == 6755366276696785)
weme_jack_sub2 <- weme_jack_sub2 %>% filter(date > as.POSIXct("2018-02-15 12:00:00") & date < as.POSIXct("2018-02-15 16:00:00"))

weme_jacks2 <- weme_jack_sub2 %>% 
  mutate(highlight_flag = ifelse(date == as.POSIXct("2018-02-15 13:49:44"), T, F)) %>% 
  ggplot(aes(x=date, y=value)) +
  geom_point(aes(color = highlight_flag), show.legend = FALSE) +
  scale_color_manual(values = c('#595959', 'red'))+
  theme_bw() +
  ggtitle("HR before, during, after squats") +
  xlab(" ") +
  ylab(" ")
print(weme_jacks2)

#Let's go back to RHR w/ medians!
noOut_dailyHR_summary <- no_HR_outliers %>% 
  group_by(userId,day) %>%
  dplyr::summarize(average_HR=mean(value), median_HR = median(value),
                   Standard_Dev=sd(value))
noOut_dailyHR_summary<-data.table(noOut_dailyHR_summary)
noOut_dailyHR_summary[,index:= order(day), by="userId"]

HR_medians <- ggplot(noOut_dailyHR_summary, aes(x=index, y=median_HR, colour=userId, group = userId)) +
  geom_point() + geom_line() +
  theme_bw() +
  ggtitle("Median HR per day per user") +
  xlab("Number of days") +
  ylab("Heart rate (beats/min)")
  #scale_color_hue(labels= index(unique(UserInfo_HR$userId)))
# geom_errorbar(aes(ymin=average_HR-Standard_Dev, ymax=average_HR+Standard_Dev),
...
noOut_HRsummary <- group_by(no_HR_outliers, userId) %>% 
  dplyr::summarize(average_HR = mean(value), median_HR=median(value), stan_dev=sd(value))
summary(noOut_HRsummary)

total_median <- ggplot(noOut_HRsummary, aes(x=userId, y=median_HR)) +
...
#Delta HR @ exercise
wemeDHR_plot <- all_HR_weme %>%
  mutate(highlight_flag = ifelse(difference > 0, T, F)) %>%
  ggplot(aes(x=animationName, y=delta_HR_daily)) +
  geom_point(aes(color = highlight_flag), show.legend = TRUE) +
  labs(color= "Within...")+
  scale_color_manual(labels= c("3 min before \n exercise", "5 min after \n exercise"), 
                     values = c('#595959', 'red'))+
  theme_bw() +
  xlab("Exercise")+
  ylab("Delta HR (bpm)")+
...
#Looking into how exercise on day affects RHR (both median and other proc)
freq_weme$day <- as.Date(freq_weme$day, format="%Y-%m-%d")
noOut_dailyHR_summary$day <- as.Date(noOut_dailyHR_summary$day, format="%Y-%m-%d")
wemefreqvsHR <- merge(noOut_dailyHR_summary, freq_weme[,c("userId", "day", "entries_on_day")], 
                      by=c("userId", "day"), all.x = TRUE)
wemefreqvsHR$exercised <- ifelse(wemefreqvsHR$entries_on_day!=0, 1, 0)
wemefreqvsHR[is.na(wemefreqvsHR)] <- 0
pers1 <- wemefreqvsHR[wemefreqvsHR$userId==6755366276694823,]

#title: Relationship between SD and Exercise entries per day
# fitA <- lm(Standard_Dev ~ entries_on_day, data = pers1)
# SDvsfit <- ggplot(pers1,
#                      aes(x=entries_on_day, y=Standard_Dev))+
#   geom_point()+
#   stat_smooth(method = 'lm', aes(group = 'linear'), se = FALSE) +
#   theme_bw()+
#   xlab("Exercise Entries per day")+
#   ylab("Standard Deviation of daily HR")+
#   #scale_color_hue(labels= index(unique(UserInfo_HR$userId)))+
#   #scale_x_discrete(labels=as.factor(index(unique(UserInfo_HR$userId))))
#   labs(title = paste("R^2 = ",signif(summary(fitA)$adj.r.squared, 3), ",",
#                      " P =",signif(summary(fitA)$coef[2,4], 4)))
# print(SDvsfit)

#no zeros (ask if ok)
fitAa <- lm(Standard_Dev ~ entries_on_day, data = pers1[pers1$entries_on_day!=0,])
SDvsfit2 <- ggplot(pers1[pers1$entries_on_day!=0,],
                  aes(x=entries_on_day, y=Standard_Dev))+
  geom_point()+
  stat_smooth(method = 'lm', aes(group = 'linear'), se = FALSE) +
  theme_bw()+
  ...
  #scale_color_hue(labels= index(unique(UserInfo_HR$userId)))+
  #scale_x_discrete(labels=as.factor(index(unique(UserInfo_HR$userId))))
  labs(title = paste("R^2 = ",signif(summary(fitAa)$adj.r.squared, 3), ",",
                     " P =",signif(summary(fitAa)$coef[2,4], 4)))
print(SDvsfit2)

fitCc <- lm(median_HR ~ entries_on_day, data = pers1[pers1$entries_on_day!=0,])
MedianvsfitCc <- ggplot(pers1[pers1$entries_on_day!=0,],
                   aes(x=entries_on_day, y=median_HR))+
  geom_point()+
  stat_smooth(method = 'lm', aes(group = 'linear'), se = FALSE) +
  theme_bw()+
  xlab("Exercise Entries per day")+
  ...
  #scale_color_hue(labels= index(unique(UserInfo_HR$userId)))+
  #scale_x_discrete(labels=as.factor(index(unique(UserInfo_HR$userId))))
  labs(title = paste("R^2 = ",signif(summary(fitCc)$adj.r.squared, 3), ",",
                     " P =",signif(summary(fitCc)$coef[2,4], 4)))
print(MedianvsfitCc)

##all data
fitB <- lm(Standard_Dev ~ entries_on_day, data = wemefreqvsHR[wemefreqvsHR$entries_on_day!=0,])
SDvsWeme <- ggplot(wemefreqvsHR[wemefreqvsHR$entries_on_day!=0,],
                     aes(x=entries_on_day, y=Standard_Dev, colour=userId))+
  geom_point()+
  stat_smooth(method = 'lm', aes(group = 'linear'), se = FALSE) +
  theme_bw()+
  xlab("Exercise Entries per day")+
  ...
  scale_color_hue(labels= index(unique(UserInfo_HR$userId)))+
  #scale_x_discrete(labels=as.factor(index(unique(UserInfo_HR$userId))))
  labs(title = paste("R^2 = ",signif(summary(fitB)$adj.r.squared, 3), ",",
                     " P =",signif(summary(fitB)$coef[2,4], 4)))
print(SDvsWeme)

fitC <- lm(median_HR ~ entries_on_day, # + (1|userId), 
             data = wemefreqvsHR[wemefreqvsHR$entries_on_day!=0,])
summary(fitC)
MedvsWeme <- ggplot(wemefreqvsHR[wemefreqvsHR$entries_on_day!=0,],
                   aes(x=entries_on_day, y=median_HR, colour=userId))+
  geom_point()+
  #facet_wrap(~ userId)+
  stat_smooth(method = 'lm', aes(group = 'linear'), se = FALSE) +
  #geom_line(aes(y=fitted(fitC), colour=userId), linetype=2)+
  theme_bw()+
  xlab("Exercise Entries per day")+
  ylab("Median of daily HR")+
  scale_color_hue(labels= index(unique(UserInfo_HR$userId)))+
  #scale_x_discrete(labels=as.factor(index(unique(UserInfo_HR$userId))))
  labs(title = paste("R^2 = ",signif(summary(fitC)$adj.r.squared, 3), ",",
